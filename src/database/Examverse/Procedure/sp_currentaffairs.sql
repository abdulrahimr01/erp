...
--insert
IF p_id=0 THEN
INSERT INTO currentaffairs(date,catagory,slug,content,created_at,updated_at,isactive,actionby,actiondate,title)
VALUES(p_date,p_catagory,p_slug,p_content,p_created_at,p_updated_at,p_isactive,p_actionby,p_actiondate,p_title);
END IF;

--update
IF p_id>0 AND EXISTS (SELECT 1 FROM currentaffairs WHERE id=p_id) THEN
UPDATE currentaffairs
SET date=p_date,catagory=p_catagory,slug=p_slug,content=p_content,created_at=p_created_at,updated_at=p_updated_at,isactive=p_isactive,actionby=p_actionby,actiondate=p_actiondate,title=p_title
WHERE id=p_id;
END IF;
