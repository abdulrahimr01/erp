...
--insert
IF p_id=0 THEN
INSERT INTO courseVideos(title,youtubevideoid,description,isactive,actionby,actiondate)
VALUES(p_title,p_youtubevideoid,p_description,p_isactive,p_actionby,p_actiondate);
END IF;

--update
IF p_id>0 AND EXISTS (SELECT 1 FROM courseVideos where id=p_id) THEN
UPDATE courseVideos SET title=p_title,youtubevideoid=p_youtubevideoid,description=p_description,
